...
export async function enviarRecordatorios(correoPrueba) {
  ...
  for (const key of Object.keys(porJugador)) {
    ...
    const esPrueba = Boolean(correoPrueba);
    const destinatarioFinal = esPrueba ? correoPrueba : j.email;
    const asunto = esPrueba
      ? `[PRUEBA] Iría a ${j.nombre} <${j.email}>`
      : 'Recordatorio de cuota pendiente — Club Alianza Francesa';
    ...
    await transporter.sendMail({
      ...
      text:
        ...
        `Nota: los montos indicados son generales y no consideran acuerdos de pago ` +
        `especiales (por ejemplo, cuota parcial o un acuerdo distinto por no haber ` +
        `participado en algún trimestre). Si tienes un acuerdo particular, este correo ` +
        `puede no reflejarlo con exactitud — cualquier duda, consúltalo con el dirigente.\n\n` +
